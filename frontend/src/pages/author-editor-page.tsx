        ...
        <div className="mt-6 grid items-start gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <section>
            ...
            <div className="grid items-start gap-6 xl:grid-cols-[minmax(320px,0.54fr)_minmax(0,1fr)]">
              ...
              <div className="xl:sticky xl:top-24"><LiveArticlePreview article={author.book.number_mode === "none" ? { ...draft, number: "" } : draft} language={language} onImagePositionChange={(imagePosition) => { if (!editingLocked) setDraft((current) => ({ ...current, imagePosition })); }} onImagePageChange={(imagePage) => { if (!editingLocked) setDraft((current) => ({ ...current, imagePage })); }} onImageSizeChange={(imageSize) => { if (!editingLocked) setDraft((current) => ({ ...current, imageSize })); }} /></div>
            </div>
          </section>
        </div>
      </main>
