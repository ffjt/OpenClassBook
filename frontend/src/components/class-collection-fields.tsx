...
import type { ClassCollectionMode } from "@/repositories/bookRepository";
          ...
          <div className="mt-4 rounded-xl border border-border bg-muted/20 p-4">
            ...
            {rules.mode === "fixed" ? (
              ...
              <div className="mt-4">
                ...
                <p className="mt-3 rounded-lg bg-background px-3 py-2 text-xs text-muted-foreground"><span className="font-semibold text-foreground">{text.preview}: </span>{rules.prefix}<span className="mx-1 rounded border border-dashed border-blue-500/50 px-2 py-0.5 text-blue-500">{text.blankPreview}</span>{rules.suffix}</p>
              </div>
            )}
          </div>
        ) : null}
